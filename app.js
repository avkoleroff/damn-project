(function () {
  ...
  const REVIEW_ADVERBS = { 1: ["ужасно"], 2: ["плохо"], 3: ["нормально", "неплохо"], 4: ["хорошо", "неплохо"], 5: ["отлично", "замечательно"] };
  function getRandomReviewAdverb(rating) {
    const arr = REVIEW_ADVERBS[Math.round(Number(rating))] || ["нормально"];
  ...
  (function seedDemoReviews() {
    try {
      ...
      const fakeReviews = [
        { author: "Ирина К.", rating: 5 }, { author: "Андрей М.", rating: 4 }, { author: "Ольга В.", rating: 5 },
        { author: "Дмитрий С.", rating: 4 }, { author: "Мария П.", rating: 5 }, { author: "Сергей Л.", rating: 4 },
        { author: "Елена Т.", rating: 5 }, { author: "Никита К.", rating: 3 }, { author: "Анна Р.", rating: 5 },
        { author: "Павел Д.", rating: 4 }
      ];
      Object.keys(BASE_USERS).forEach((key) => {
        ...
        while (existing.length < need) {
          ...
          const copy = { ...r, date: d.toISOString().slice(0, 10), text: getRandomReviewAdverb(r.rating) };
          existing.push(copy);
        }
        all[key] = existing;
      });
  ...
  function saveReview(workerUsername, review) {
    ...
    const text = (review.text && String(review.text).trim()) ? String(review.text).trim() : getRandomReviewAdverb(review.rating);
  ...
  function getWorkingHoursForDay(worker, dayNum) {
    ...
    if (grid && Array.isArray(grid)) {
      return gridStateToIntervals(grid);
    }
  ...
  function getDateForDayOfWeek(dayNum) {
    const now = new Date();
  ...
  function getFreeIntervalsForWorkerDay(workerUsername, dayNum, dateStr) {
    ...
    if (hasGrid) {
      return gridStateToIntervals(w.timetableGrid[dayNum])
        .map((seg) => ({ start: timeToMinutes(seg.start), end: timeToMinutes(seg.end) }));
    }

    const workSegments = getWorkingHoursForDay(w, dayNum);
    const busySlots = getBusySlots(workerUsername, dateStr);
    let free = workSegments.map((s) => ({ start: timeToMinutes(s.start), end: timeToMinutes(s.end) }));
    busySlots.forEach((busy) => {
      const bStart = timeToMinutes(busy.start);
      const bEnd = timeToMinutes(busy.end);
      const next = [];
      free.forEach((f) => {
        if (bEnd <= f.start || bStart >= f.end) next.push(f);
        else {
          if (f.start < bStart) next.push({ start: f.start, end: Math.min(f.end, bStart) });
          if (f.end > bEnd) next.push({ start: Math.max(f.start, bEnd), end: f.end });
        }
      });
      free = next;
    });
  ...
  function workerMatchesTimeFilter(username, dayNum, reqStartMin, reqEndMin, strict) {
    const d = getDateForDayOfWeek(dayNum);
    const dateStr = d.getFullYear() + "-" + String(d.getMonth() + 1).padStart(2, "0") + "-" + String(d.getDate()).padStart(2, "0");
    const free = getFreeIntervalsForWorkerDay(username, dayNum, dateStr);
    if (strict) {
      return free.some((f) => f.start <= reqStartMin && f.end >= reqEndMin);
    } else {
      return free.some((f) => f.start < reqEndMin && f.end > reqStartMin);
    }
  }
  ...
  function showWorkerCabinet(username) {
    ...
    if (cabinetTimetableEl) {
      const dayLabels = { 0: "Вс", 1: "Пн", 2: "Вт", 3: "Ср", 4: "Чт", 5: "Пт", 6: "Сб" };
    ...
    getReviews(username).forEach((r) => {
      ...
      div.innerHTML = "<div class=\"review-author\">" + escapeHtml(authorLabel) + " · " + r.date + " · ★" + r.rating + "</div><div>" + (r.text && r.text.trim() ? escapeHtml(r.text) : (REVIEW_ADVERBS[Math.round(Number(r.rating))] || ["нормально"])[0]) + "</div>";
  ...
  function getFilteredWorkers() {
    ...
    // Фильтр по времени: отдельные настройки на каждый день (включаются чекбоксом).
    for (let dayNum = 0; dayNum <= 6; dayNum++) {
      const enableEl = document.getElementById("filter-time-enable-" + dayNum);
      if (!enableEl || !enableEl.checked) continue;
      const startEl = document.getElementById("filter-time-start-" + dayNum);
      const endEl = document.getElementById("filter-time-end-" + dayNum);
      const modeEl = document.getElementById("filter-time-mode-" + dayNum);
      const startVal = startEl && startEl.value ? startEl.value : "08:00";
      const endVal = endEl && endEl.value ? endEl.value : "22:00";
      const reqStartMin = timeToMinutes(startVal);
      const reqEndMin = timeToMinutes(endVal);
      const strict = !(modeEl && modeEl.value === "nonstrict");
      list = list.filter((w) => workerMatchesTimeFilter(w.username, dayNum, reqStartMin, reqEndMin, strict));
    }
  ...
  function buildTimetableWrap(workerUsername) {
    ...
    for (let hour = startHour; hour < endHour; hour++) {
      ...
      for (let dayIdx = 0; dayIdx < 7; dayIdx++) {
        ...
        if (hasGrid) {
          isBusy = cellState === 2;
          isWorking = cellState === 1;
        } else {
          isWorking = workSegments.some((seg) => slotOverlaps(slotStartMin, slotEndMin, seg.start, seg.end));
          isBusy = false;
        }
        const cell = document.createElement("div");
  ...
  function renderResultsDetail(workerUsername) {
    ...
    if (isUser && !isSelf) {
      ...
      submitBtn.onclick = function() {
        ...
        saveReview(workerUsername, { rating, text: (textInput && textInput.value) ? textInput.value.trim() : "" });
  ...
  const timeFilterEls = [];
  for (let dayNum = 0; dayNum <= 6; dayNum++) {
    ["enable", "start", "end", "mode"].forEach((k) => {
      const el = document.getElementById("filter-time-" + k + "-" + dayNum);
      if (el) timeFilterEls.push(el);
    });
  }

  [filterKind, filterCity, filterPriceMin, filterPriceMax, sortBy, ...timeFilterEls].forEach((el) => {
